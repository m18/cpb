...
func TestQueryParserParseInMessageArgs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			driver:           DriverPostgres,
			query:            "select * from test where foo_col = $foo(1, 'one')",
		...
		},
		{
			...
			driver:           DriverPostgres,
			query:            "select * from test where foo_col = $foo(1, 'one') and bar_col = $bar(2, 'two')",
		...
		},
		{
			...
			driver:           DriverPostgres,
			query:            "select * from test where foo_col = $foo(1, 'o\\'one')",
			expectedQuery:    "select * from test where foo_col = $1",
			expectedArgCount: 1,
		},
		{
			...
			driver:           DriverPostgres,
			query:            "select * from test where foo_col = $foo(1, 'o\\'one') and bar_col = $bar(2, 'o\\'two')",
			expectedQuery:    "select * from test where foo_col = $1 and bar_col = $2",
			expectedArgCount: 2,
		},
		{
			...
			driver:           DriverPostgres,
			query:            "select * from test where foo_col = $foo(1, 'one \\\" two')",
			expectedQuery:    "select * from test where foo_col = $1",
			expectedArgCount: 1,
		},
		{
		...
		{
			desc:   "invalid, single param, wrong arg type",
			driver: DriverPostgres,
			query:  "select * from test where foo_col = $foo('not a number', 'one')",
			err:    true,
		},
		{
		...
		},
		{
			...
			driver: DriverPostgres,
			query:  "select * from test where foo_col = $foo(1, 'one') and bar_col = $bar('not a number', 'two')",
			err:    true,
		},
		{
			desc:   "invalid, multiple params, wrong arg count",
			driver: DriverPostgres,
			query:  "select * from test where foo_col = $foo(1, 'one') and bar_col = $bar(2)",
			err:    true,
		},
	}
	for _, test := range tests {
